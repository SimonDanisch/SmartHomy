...
function Sockets.send(success_callback, device::DeviceConnection, message; wait_for_result=false)
    put!(device.sent_lock) do
        try
            socket = Sockets.connect(device.ip)
            send_encrypted(socket, message)
            if wait_for_result
                success_callback(decrypt(read(socket)))
            else
                success_callback()
                close(socket)
            end
...
function query_devices()
    ...
    queries = Dict(
        ...
        "smartlife.iot.smartbulb.lightingservice" => Dict("get_light_state" => nothing),
    )
